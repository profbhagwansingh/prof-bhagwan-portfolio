...
export function GalleryPage() {
  ...
  useEffect(() => {
    ...
    const loadFolders = async () => {
      try {
        const { data } = await api.get("/api/gallery/folders");
        if (active) {
    ...
    const loadFiles = async () => {
      ...
      try {
        if (activeFolder === "ALL") {
          ...
          const promises = folders.map(f => api.get(`/api/gallery/files?folder=${encodeURIComponent(f.folder)}`));
          ...
          const { data } = await api.get(`/api/gallery/files?folder=${encodeURIComponent(activeFolder)}`);
