...
interface FolderInfo { folder: string; label: string; count: number; sortOrder: number; }
...
function FileGrid({
  ...
  onRename, onDelete, emptyLabel, onMove, onMoveToIndex,
}: {
  ...
  onMove?: (fileUrl: string, direction: 'left' | 'right') => Promise<void>;
  onMoveToIndex?: (fileUrl: string, newIndex: number) => Promise<void>;
}) {
  ...
  const [editingIndexFile, setEditingIndexFile] = useState<string | null>(null);
  const [editIndexVal, setEditIndexVal] = useState("");

  const startEditIndex = (fileUrl: string, currentIndex: number) => {
    if (!onMoveToIndex) return;
    setEditingIndexFile(fileUrl);
    setEditIndexVal((currentIndex + 1).toString());
  };

  const submitEditIndex = async () => {
    if (!editingIndexFile || !onMoveToIndex) {
      setEditingIndexFile(null);
      return;
    }
    const newIndex = parseInt(editIndexVal, 10) - 1;
    if (!isNaN(newIndex)) {
      try {
        await onMoveToIndex(editingIndexFile, newIndex);
      } catch {}
    }
    setEditingIndexFile(null);
  };
          ...
          <div key={fileUrl} className="relative group rounded-xl border border-[var(--border)] overflow-hidden bg-[var(--bg-card)] shadow-sm hover:shadow-md transition-all">
            <div className="aspect-[4/3] w-full relative">
              ...
              <div className="absolute top-2 left-2 flex items-center justify-center">
                {editingIndexFile === fileUrl ? (
                  <input
                    value={editIndexVal}
                    onChange={(e) => setEditIndexVal(e.target.value)}
                    onBlur={submitEditIndex}
                    onKeyDown={(e) => { if (e.key === 'Enter') submitEditIndex(); if (e.key === 'Escape') setEditingIndexFile(null); }}
                    className="w-8 h-6 text-center text-[10px] font-bold rounded bg-white text-black focus:outline-none focus:ring-1 focus:ring-primary-500"
                    autoFocus
                  />
                ) : (
                  <div 
                    onClick={() => startEditIndex(fileUrl, index)}
                    title="Click to edit order"
                    className="w-6 h-6 rounded-lg bg-black/50 backdrop-blur-sm text-white flex items-center justify-center text-[10px] font-bold cursor-pointer hover:bg-black/70 transition-colors"
                  >
                    {index + 1}
                  </div>
                )}
              ...
              <div className="absolute top-2 right-2 flex gap-1.5 opacity-0 group-hover:opacity-100 transition-all">
                {onMove && index > 0 && (
                  <button onClick={() => onMove(fileUrl, 'left')} className="w-7 h-7 rounded-lg bg-indigo-500/90 text-white flex items-center justify-center hover:bg-indigo-600 transition-colors shadow-sm" title="Move Left"><ArrowLeft className="w-3.5 h-3.5" /></button>
                )}
                {onMove && index < files.length - 1 && (
                  <button onClick={() => onMove(fileUrl, 'right')} className="w-7 h-7 rounded-lg bg-indigo-500/90 text-white flex items-center justify-center hover:bg-indigo-600 transition-colors shadow-sm" title="Move Right"><ArrowRight className="w-3.5 h-3.5" /></button>
                )}
...
export default function GalleryAdminPage() {
  ...
  const fetchFolders = useCallback(async () => {
    ...
    try {
      ...
      list.forEach((f: FolderInfo, i: number) => { orders[f.folder] = f.sortOrder ?? i; });
      setFolderOrders(orders);
      setOrderDirty(false);
    } catch { }
  ...
  const handleSlideshowMove = async (fileUrl: string, direction: 'left' | 'right') => {
    const currentIndex = slideshowFiles.indexOf(fileUrl);
    if (currentIndex === -1) return;
    
    const newOrder = [...slideshowFiles];
    if (direction === 'left' && currentIndex > 0) {
      const temp = newOrder[currentIndex - 1];
      newOrder[currentIndex - 1] = fileUrl;
    ...
    setSlideshowFiles(newOrder); 
    try {
      await api.patch('/api/gallery/admin/slideshow-files/order', { order: newOrder });
      toast({ type: "success", title: "Order saved" });
    } catch {
      fetchSlideshow(); 
  ...
  const handleSlideshowMoveToIndex = async (fileUrl: string, newIndex: number) => {
    const currentIndex = slideshowFiles.indexOf(fileUrl);
    if (currentIndex === -1 || newIndex < 0 || newIndex >= slideshowFiles.length || currentIndex === newIndex) return;
    
    const newOrder = [...slideshowFiles];
    newOrder.splice(currentIndex, 1);
    newOrder.splice(newIndex, 0, fileUrl);
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              {orderDirty && (
                ...
                >
                  {isSavingOrder ? <Loader2 className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} Save Order
                </button>
              )}
              ...
              {folders.map((f) => {
                ...
                <div key={f.folder} className="card-base p-5 flex items-center gap-4 hover:border-primary-300 hover:shadow-md transition-all text-left group">
                  <input
                    type="number"
                    min="0"
                    value={folderOrders[f.folder] ?? 999}
                    onChange={(e) => {
      ...
      {activeTab === "slideshow" && (
        <div className="space-y-6">
          ...
          <FileGrid
            ...
            onMove={handleSlideshowMove}
            onMoveToIndex={handleSlideshowMoveToIndex}
          />
        </div>
      )}
