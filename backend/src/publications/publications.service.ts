...
export class PublicationsService {
    ...
    async getBookChapters() {
        return this.prisma.bookChapter.findMany({ where: { isActive: true }, orderBy: [{ year: 'desc' }, { sortOrder: 'asc' }] });
    }

    async getAllBookChapters() {
        return this.prisma.bookChapter.findMany({ orderBy: [{ year: 'desc' }, { sortOrder: 'asc' }] });
    }

    async upsertBookChapter(data: any) {
        if (data.id) return this.prisma.bookChapter.update({ where: { id: data.id }, data });
        return this.prisma.bookChapter.create({ data });
    }

    async deleteBookChapter(id: string) {
